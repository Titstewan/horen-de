...
\subsubsection{Titel} Titel fungieren als Substantivmodifikatoren und werden daher nicht dekliniert, wenn sie mit Eigennamen verwendet werden. Der Dativ von \N{Karyu Pawl} \glqq Lehrer Paul\grqq ist \N{Karyu Pawlur}. \index{Kasus!mit Titeln}
...
\subsubsection{} Der Superlativ wird mit \N{\ACC{fra}to} \E{als alle} (wörtl. \E{\glqq im Vergleich zu allen\grqq}) gebildet, \Npawl{fìsyulang arim lu hì'i frato} \E{diese gelbe Blume ist die kleinste (von allen)}. \index{frato@\textbf{frato}}
\index{Adjektiv!Superlativ}

\subsubsection{} Vergleiche, die auf Gemeinsamkeiten basieren, \glqq so groß wie ein Baum\grqq, werden mit dem Idiom `\N{nìftxan} -- Adj. -- \N{na} -- Substantiv oder Pronomen' formuliert, wie in \Npawl{oe lu nìftxan sìltsan na nga} \E{ich bin genauso gut wie du.} Wenn der Vergleichspunkt ein Pronomen oder ein definites Substantiv ist, das bereits Teil des Diskurses ist, kann der Topik-Fall verwendet werden, \Npawl{ngari lu oe nìftxan sìltsan}. Diese Konstruktion kann auch mit Adverben verwendet werden. \label{syntax:adj-eql-comp}
...
\subsection{Fko} Das Indefinitpronomen \N{fko} entspricht im Grunde dem deutschen \glqq man\grqq, wie in \E{man sagt solche Dinge nicht}: \Npawl{Tsat ke tsun fko yivom} \E{das kann man nicht essen}, \Npawl{tsun fko sivar hänit fte payoangit stivä'nì} \E{man kann ein Netz benutzen, um einen Fisch zu fangen}.
\index{fko@\textbf{fko}}

\subsubsection{} \N{Fko} wird auch dort verwendet, wo das Deutsche bei allgemeinen Aussagen ein \glqq sie\grqq (bzw. alternativ ein \glqq man\grqq) verwenden würde, wie in \Npawl{\uwave{plltxe fko} san ngaru lu mowan Txilte ulte poru nga} \E{\uwave{sie sagen / man sagt}, dass du Txilte magst und umgekehrt}.\footnote{Frommers Übersetzung lautet: \E{Ich höre, du magst Txilte und umgekehrt}.}

\subsubsection{} \N{Fko} kann mit dem Passiv übersetzt werden, wenn der Handelnde\footnote{Der Handelnde bzw. das Agens eines Verbs im Passiv ist die Person oder Sache, die in deutschen Passivkonstruktionen meist nach der Präposition \glqq von\grqq steht, wie in \E{ich wurde \uwave{vom Auto} angefahren}.} belebt ist, wie in der Redewendung \N{oeru syaw fko Wìlyìm} \E{mein Name ist William / ich werde William genannt}, \Npawl{tsalì'uri fko pamrel si fyape?} \E{wie wird dieses Wort geschrieben / wie schreibt man dieses Wort?}
\label{syn:prn:fko}
...
\begin{interlin}
	...
	alles-\I{pat} \glqq man\grqq-\I{agt} zerstören\INF{\I{ipfv}} \\
	\trans{Alles wird zerstört.}
\end{interlin}

\subsection{Sno} \index{sno@\textbf{sno}}
Das Reflexivpronomen \N{sno} bezieht sich auf das Subjekt oder den Handelnden des Satzes, in dem es steht. Im Genitiv (\N{sneyä}) kann es mit \E{sein/ihr/deren eigene/r/s} übersetzt werden. Es wird verwendet, um die Doppeldeutigkeit von Sätzen wie \glqq er hat sein Essen zubereitet\grqq zu klären -- hier kann es sonst unklar sein, ob sich \glqq sein\grqq auf die Person bezieht, die das Essen zubereitet, oder auf jemand anderen:
...
\label{syn:cases}
\subsection{Absolutiv} Der unmarkierte Kasus\footnote{Anm. d. Ü.: Frommer bezeichnet diesen Fall als \glqq Subjective\grqq; die deutsche linguistische Terminologie, der die vorliegende Übersetzung folgt, verwendet den Begriff \glqq Absolutiv\grqq (vgl. die Anmerkung zu \horenref{morph:decl}).} wird für das Subjekt intransitiver Sätze, das Prädikativ in Prädikatskonstruktionen (\horenref{syn:predicates}) sowie mit Adpositionen verwendet. \index{Kasus!Absolutiv}\index{Subjekt}
...
\subsubsection{} Das Verb \N{lu} bildet mit dem Dativ ein besitzanzeigendes Idiom, wo das Deutsche das Verb \glqq haben\grqq verwendet, \N{lu oeru ikran} \E{ich habe eine Banshee}. In dieser Konstruktion steht das Verb üblicherweise am Anfang des Satzes. \index{Kasus!Dativ!mit \textbf{lu}}
...
Na'vi hat mindestens drei Fälle, die im weitesten Sinne als \glqq Subjekt\grqq verstanden werden können: Der Absolutiv, der Agens-Fall und manchmal sogar der Dativ können sich einem subjekt-ähnlichen Verhalten annähern, etwa in \N{fì'u sunu \uwave{oer}} \E{\uwave{ich} mag dies}.
...
\noindent Wenn eine Klarstellung erforderlich ist, wird einfach das entsprechende Pronomen verwendet. Beispielsweise könnte man im ersten Beispiel oben auch \N{oe zene tivätxaw} sagen. Man beachte auch im zweiten Beispiel, dass das \glqq Subjekt\grqq im ersten Satz im Dativ steht, \N{oer}, während im zweiten Satz das ausgelassene Subjekt im Agens stehen würde, \N{oel}.
...
%\N{aylì'u na ayskxe \uwave{mì te'lan}} \E{the words (are) like stones
	...
	%has \uwave{on his back}}.  It also describes location in (or \glqq on\grqq)
...
%\subsection{Ta} \E{From} (various uses). \Npawl{Oeri ta peyä fahew
	%akewong ontu teya längu} \E{my nose is full of (\glqq from\grqq) his alien
	%smell}.
...
%
%\subsection{Wä (+)} \E{Against} (as in \glqq fight against\grqq).
...
\subsubsection{\glqq bisher nicht\grqq} Die Negation, \N{ke li}, bedeutet \glqq noch nicht, bisher nicht\grqq und verwendet doppelte Verneinung (\horenref{syn:neg:pleon}), \N{fo ke li ke polähem} \E{sie sind noch nicht angekommen}.
...
\subsubsection{Befehle} Bei Imperativen zeigt \N{li} besondere Dringlichkeit an: \Npawl{Ngal mi fìtsengit terok srak? Li kä!} \E{Du bist noch hier? Geh schon!} Mit \N{ko} (\horenref{syn:particle:ko}), \N{li ko} (betont auf \N{li}), bedeutet es \glqq nun, dann mal los\grqq, oder \glqq lasst uns loslegen\grqq.

\subsubsection{Zögerlichkeit} In Antworten vermittelt es ein etwas zögerliches \glqq Ja\grqq, ähnlich wie das deutsche \glqq sozusagen\grqq oder \glqq nun ja\grqq:
...
\noindent Die Negation \N{ke li} bedeutet hierbei in etwa \glqq nicht wirklich\grqq. 

\subsubsection{Mit \glqq sre\grqq} Wenn \N{li} mit der Adposition \N{sre} verwendet wird, bedeuten sie \glqq bis spätestens, aber nicht danach\grqq, \Npawl{kem si li trraysre} \E{erledige das bis spätestens morgen}. \label{syn:li:sre} Wenn \N{sre} vor dem Substantiv steht, verbindet es sich mit \N{li} zu \N{\ACC{li}sre}, das wie \N{sre} lenisiert, \Npawl{kem si lisre srray} \E{erledige das bis spätestens morgen}.
...
\end{quotation}
\subsubsection{\glqq Beide\grqq} Mit dem Dual bedeutet \N{nìwotx} \E{beide}:
...
\subsection{Punktuelles Perfekt} Das Perfekt wird in mehreren Ausdrücken verwendet, um anzuzeigen, dass das Ereignis in einem Augenblick (punktuell) eingetreten ist, \N{tslolam} \E{verstanden, kapiert}, \N{rolun} \E{gefunden!}. Frommer sagt, \N{tolel}, \E{(ich) hab's!}, stehe für einen \glqq Geistesblitz\grqq.
...
\subsection{Nahe Temporaldeixis} Die Zeitformen der nahen Deixis markieren Ereignisse in der \glqq nahen\grqq Vergangenheit oder Zukunft, wobei \glqq Nähe\grqq kein absoluter Maßstab ist, sondern vom Kontext und der Perspektive des Sprechers bestimmt wird.

\subsubsection{\glqq Bald\grqq} Das Adverb \N{ye'rìn} \E{bald} kann zwar bei Verben mit dem Futurinfix \N{\INF{ay}} stehen; bei Verben, die mit der nahen Zukunft, \N{\INF{ìy}}, markiert sind, ist es aber redundant.
\LNForum{25/10/2022}{https://forum.learnnavi.org/index.php?msg=679687}
...
\begin{interlin}
	...
	\trans{Wir werden nicht kämpfen}, i. S. v. \E{\glqq Ich beabsichtige, dass wir nicht kämpfen.\grqq}
\end{interlin}
...
\subsubsection{} Im Deutschen, das kein Gerundium kennt, wird anstelle der Substantivierung des Verbs meist eine Infinitivkonstruktion mit \glqq zu\grqq verwendet (\glqq einen Marathon \uwave{zu laufen} ist schwierig\grqq). Auch im Na'vi kann das entsprechende Verb in einen mit \N{fì’u} oder \N{tsa'u} eingeleiteten Nebensatz ausgelagert werden (\horenref{syn:clause-nom}), \Npawl{\uwave{fwa yom teylut} 'o' lu}, \E{\uwave{Teylu zu essen} macht Spaß}. \index{Gerundium!Gebrauch}
...
\noindent Dieses Muster kann auch als \glqq Antipassiv\grqq bezeichnet und im Na'vi frei verwendet werden.
...
Einfache Fragen, die mit \glqq Ja\grqq oder \glqq Nein\grqq beantwortet werden können, werden mit der Partikel \N{srak(e)} gekennzeichnet, die am Anfang oder am Ende des Satzes steht. Steht sie am Ende, wird sie normalerweise zu \N{srak} abgekürzt, das längere \N{srake} steht am Satzanfang, \Npawl{ngaru lu fpom srak?} \E{geht es dir gut?}.
...
\subsection{Refrainfragen} Die Na'vi-Refrainfrage (dt. \glqq oder?\grqq, \glqq nicht wahr?\grqq; engl. \glqq right?\grqq; frz. \glqq n'est-ce pas?\grqq) wird entweder mit \N{kefya srak} oder einfach \N{kefyak} markiert (letztlich abgeleitet von \N{ke lu fìfya srak?} \E{ist es nicht so?}).
...
\subsection{Evidenz} Das Infix \N{\INF{ats}}, das ebenfalls Position 2 belegt, wird verwendet, um eine Vermutung zu kennzeichnen, die auf der Grundlage einer Evidenz angestellt wird,\footnote{Im Deutschen wird dieser Modus oft mit \glqq müssen\grqq (z. B. \glqq es muss geregnet haben\grqq, \glqq er muss Probleme mit seinen Hausaufgaben haben\grqq) oder dem Futur (\glqq das wird schon richtig sein\grqq) ausgedrückt.} \Npawl{'uol ikranit txopu sleykolatsu, taluna po tsìk yawo} \E{irgendetwas muss die Banshee erschreckt haben, denn sie erhob sich plötzlich in die Luft.} \index{Evidenz}
...
\subsubsection{} Bei \N{si}-Verben steht \N{ke} vor \N{si}, wie in \N{po pamrel ke si} \E{er schreibt nicht}. Der Satzakzent verlagert sich vom Substantiv- bzw. Adjektivelement des \N{si}-Verbs zu \N{ke}: \N{pamrel \ACC{ke} si} (siehe auch
...
\subsubsection{} Im Na'vi werden Finalsätze in Situationen verwendet, in denen das Deutsche eine Infinitivkonstruktion mit \glqq zu\grqq verwenden würde, \Npawl{pxiset ke lu oeru krr \uwave{fte} tì'eyngit \uwave{tivìng}} \E{momentan habe ich nicht die Zeit, eine Antwort \uwave{zu geben}}.
...
\subsection{Verweishierarchie} Wenn der Kopf\footnote{Der Kopf des Relativsatzes ist das Substantiv im Hauptsatz, das vom Relativsatz modifiziert wird. Es hat eine syntaktische Rolle sowohl im Haupt- als auch im Relativsatz: In dem Satz \glqq ich sehe den Mann, der rennt\grqq ist das Wort \glqq Mann\grqq das direkte Objekt des Hauptsatzes \glqq ich sehe den Mann\grqq, aber zugleich das Subjekt des Relativsatzes \glqq der Mann rennt\grqq. Dieses Element, das beiden Sätzen gemeinsam ist, wird manchmal auch als \glqq syntaktischer Drehpunkt\grqq bezeichnet.} eines Relativsatzes das Subjekt oder das direkte Objekt dieses Relativsatzes ist, entfällt er:
...
\subsubsection{} Das Substantiv \N{tìkin} \E{Bedarf, Bedürfnis, Erfordernis, Notwendigkeit} wird mit einem Attributivsatz für \glqq müssen\grqq verwendet, \Npawl{awngaru lu tìkin a nume nì'ul} \E{wir müssen mehr lernen} (wörtl. \glqq wir haben die Notwendigkeit, mehr zu lernen\grqq). \index{tìkin@\textbf{tìkin}!mit Attributivsatz}
Es kann auch unpersönlich verwendet werden, \N{lu tìkin a ...} \E{es besteht die Notwendigkeit für ...}

\subsection{Inhaltssätze mit Adpositionen} Einige Adpositionen können Teilsätze einleiten, wo im Deutschen eine Infinitivkonstruktion mit \glqq zu\grqq steht, \N{oe ke tsun stivawm fayfnelì'ut \uwave{luke fwa sngä'i tsngivawvìk}} \E{ich kann solche Worte nicht hören, \uwave{ohne anzufangen zu weinen}}.
...
\subsection{Allgemein} Allgemeine Bedingungen beschreiben Situationen, die universell wahr sind, wie z. B. \glqq wenn es nicht regnet, leiden Pflanzen und Tiere\grqq. \QUAESTIO{Im Na'vi wird eine allgemeine Bedingung mit \N{txo} ausgedrückt, wobei in der Bedingung der Subjunktiv und in der Konsequenz ein Indikativ ohne Tempusmarkierung verwendet wird}, \Npawl{txo fkol ke fyivel uranit paywä, zene fko slivele} \E{wenn man ein Boot nicht gegen Wasser abdichtet, muss man schwimmen.}
\index{Konditionalsatz!allgemein}
\NTeri{19/6/2012}{https://naviteri.org/2012/06/spring-vocabulary-part-3/}

\subsection{Zukunft} Im Deutschen steht oft die Bedingung im Indikativ und die Folge im Futur I; z. B. \glqq wenn du dieses machst, werde ich jenes machen\grqq. Im Na'vi hingegen steht die Bedingung im Subjunktiv und die Konsequenz in der Zukunft, \Npawl{pxan \uwave{l\INF{iv}u} txo nì'aw oe ngari / tsakrr nga Na'viru \uwave{yomt\INF{ìy}ìng}} \E{nur wenn ich deiner würdig \uwave{bin}, / \uwave{wirst} du das Volk versorgen}. \index{Konditionalsatz!Zukunft}
...
\subsection{Irrealis} Der Irrealis bzw. Kontrafaktual (d. h. ein Konditionalsatz mit einer Bedingung, die der Realität widerspricht und daher nicht (mehr) erfüllbar ist) wird mit einer eigenen Gruppe von Konjunktionen gebildet, \N{zun} \E{wenn} und \N{zel} \E{dann}. Der Subjunktiv wird in beiden Sätzen verwendet, mit den folgenden Zeitformen:
...
\noindent Für Situationen im Präsens wird der bloße Subjunktiv oder der Subjunktiv Imperfekt verwendet; für vergangene Situationen wird der Subjunktiv Perfekt oder der Subjunktiv der Vergangenheit verwendet; und für zukünftige Situationen wird der Subjunktiv der Zukunft gebraucht (siehe \horenref{morph:verb:first-position} für die Infixformen).\footnote{Anm. d. Ü.: Im Deutschen wird der Irrealis mit \glqq wenn\grqq bzw. \glqq falls\grqq und dem Konjunktiv II gebildet, z. B. \glqq wenn du ging(e)st, wäre ich traurig\grqq. Sehr häufig kann im Deutschen ganz auf \glqq wenn\grqq und \glqq dann\grqq verzichtet werden, \glqq gingest du, wäre ich traurig\grqq. Alltagssprachlich wird anstelle des Konjunktivs oft auf eine Umschreibung mit \glqq würde\grqq zurückgegriffen, \grqq würdest du gehen, würde ich traurig sein\grqq.}
...
\subsubsection{} \N{Alu} kann im Diskurs auch dazu verwendet werden, eine Neuformulierung zu markieren, wie im Deutschen \glqq das soll heißen\grqq oder \glqq mit anderen Worten\grqq, \Npawl{txoa livu, yawne lu oer Sorewn ...\ \uwave{alu} ...\ ke tsun oeng muntxa slivu} \E{Entschuldigung, aber ich liebe Sorewn ... \uwave{soll heißen} ... du und ich können nicht heiraten.}
...
\subsection{San ... sìk} Na'vi kennt keine indirekte Rede (anders als z. B. das Deutsche, wie in \glqq er sagte, \uwave{dass er gegangen sei}\grqq), sondern gibt stets den Wortlaut des Gesagten in direkter Rede wieder, wobei das Zitat zwischen den Partikeln \N{san} und \N{sìk} steht, wie in \Npawl{slä nì'i'a tsun oe pivlltxe \uwave{san Zola'u nìprrte' ne pìlok Na'viteri sìk}!} \E{aber jetzt kann ich endlich sagen: \glqq\uwave{Willkommen auf dem Blog Na'viteri}!\grqq}. \index{Zitation!direkt}
...
\begin{quotation}
	...
	\indent\E{Eytukan sagte \glqq ich werde gehen\grqq, aber ich glaube ihm nicht.}\\
	\noindent 2. \N{Poltxe Eytukan \uwave{san} oe kayä.} \E{Eytukan sagte \glqq ich werde gehen\grqq.}
\end{quotation}

\noindent Da im zweiten Satz nach dem Zitat nichts mehr gesagt wird, muss das Zitat nicht mit \N{sìk} abgeschlossen werden. Ebenso kann \N{san} weggelassen werden, sofern dies nicht zu Mehrdeutigkeiten führt, \Npawl{frawzo sìk, slä oel poet ke spaw} \E{(sie sagte) \glqq alles gut\grqq, aber ich glaube ihr nicht}. 
...
\subsection{Fragen} Indirekte Fragen werden ebenfalls direkt zitiert, \Npawl{polawm po san srake Säli holum sìk} \E{er fragte, ob Sally gegangen ist} (wörtl. \E{er fragte: \glqq ist Sally gegangen?\grqq}).
\LNWiki{24/3/2010}{https://wiki.learnnavi.org/index.php/Canon/2010/March-June\%23If_and_Whether}

\subsubsection{} Bei \N{pawm}, aber nicht bei anderen Verben des Sprechens, kann \N{san ... sìk} weggelassen werden, \Npawl{polawm po, Neytiri kä pesengne?} \E{er hat gefragt, wohin Neytiri geht} (wörtl. \E{er hat gefragt: \glqq wohin geht Neytiri?\grqq}).
\NTeri{31/8/2011}{https://naviteri.org/2011/08/reported-speech-reported-questions/}

\subsection{Transitivität} Wenn ein Verb des Sprechens \N{san ... sìk} verwendet, ist der Satz intransitiv, \N{po poltxe san srane} \E{sie sagte \glqq ja\grqq.} \index{Transitivität!mit Verben des Sprechens}
...
\subsubsection{} Wird \N{plltxe} transitiv benutzt, so darf nur \N{lì'u} und deren Derivative benutzt werden. Das Wort \N{lì'fya} ist z.B. kein gültiges Objekt.
...
\begin{quotation}
	...
	\indent\E{Sie sagte, sie wollte gehen} (wörtl. \E{Sie sagte: \glqq Ich will gehen\grqq}).\\
	\noindent\Npawl{Ngal poleng oer fmawnta po tolerkup.} \\
	\indent\E{Du hast mir gesagt, dass er gestorben sei} (wörtl. \E{Du hast mir gesagt: \glqq Er ist gestorben\grqq}).\\
	\noindent\N{Volin pol tì’eyngit a Neytiri kä pesengne.} \\
	\indent\E{Er hat gefragt, wohin Neytiri geht} (wörtl. \E{Er hat gefragt: \glqq Wohin geht Neytiri?\grqq}).
\end{quotation}
...
\subsection{Ko} Die Partikel \N{ko} wird verwendet, um Zustimmung verschiedener Art einzufordern, wo im Deutschen etwa \glqq lasst uns\grqq, \glqq meinst du nicht?\grqq, \glqq warum machst du nicht?\grqq formuliert wird. Sie steht immer am Ende eines Satzes. Im Film häufig gesagt wird etwa \Nfilm{makto ko} \E{lasst uns reiten}.
...
\subsection{Tut} Hierbei handelt es sich um eine Diskurspartikel, die bisher nur bei \glqq zurückgeworfenen\grqq Fragen in Dialogen zu beobachten ist: 
...
\subsection{Tse} Diese Rezeptionspartikel zeigt ein Zögern in einer Unterhaltung an, \E{also, nun ja}. \QUAESTIO{Im Englischen wird \glqq well\grqq in unterschiedlicher Weise mit Glück in Verbindung gebracht.}
\index{tse@\textbf{tse}}

\section{Andere erwähnenswerte Wörter}

\subsection{Sweylu} \label{syn:sweylu} Die Syntax dieses Verbs mit der Bedeutung \glqq sollte\grqq (von \N{swey lu} \E{es ist am besten}) ändert sich je nachdem, ob sich die Verpflichtung auf etwas bezieht, das noch nicht geschehen ist, oder ob sie sich auf ein Ereignis bezieht, das bereits stattgefunden hat.
...
\noindent Man beachte, dass sich dies auf eine vergangene Handlung bezieht, die tatsächlich stattgefunden hat und richtig war, und nicht auf einen unerfüllten Wunsch in der Vergangenheit (was eine andere Verwendung von \glqq sollte\grqq im Deutschen ist).
