...
\LNWiki{21/12/2009}{https://wiki.learnnavi.org/index.php/Canon\%23Extracts\_from\_various\_emails}
...
\LNWiki{20/5/2010}{https://wiki.learnnavi.org/index.php/Canon/2010/March-June\%23The\_Dual\_sounds\_of\_.22u.22}
...
\begin{center}\footnotesize
	...
	\end{tikzpicture}
\end{center} \label{lands:syllable-structure}

\subsubsection{} Da eine Silbe nicht unbedingt einen Anfangs- oder Endkonsonanten haben muss, ist es durchaus möglich, dass mehrere Vokale in einem Wort nebeneinander stehen. In diesem Fall ist jeder Vokal eine Silbe für sich, \N{muiä} [mu.i.æ], \N{ioang} [i.o.aŋ], \N{meoauniaea} [ˌmɛ.o.a.u.ni.a.ˈɛ.a].

\subsubsection{} Im Allgemeinen wird die Sequenz VKV als V.KV anstelle von VK.V getrennt; so wird \N{tsenge} als [ˈtsɛ.ŋɛ] und nicht als *[ˈtsɛŋ.ɛ] getrennt. Onomatopoie (Lautmalerei) kann dies aufheben, wenn ein Echoeffekt gewünscht ist, wie in \N{kxangangang} [ˈk'aŋ.aŋ.aŋ].
...
\subsection{Betonung}
Jedes Na'vi-Grundwort hat mindestens eine betonte Silbe, die nicht vorhersagbar ist. In sehr wenigen Fällen unterscheiden sich ansonsten identische Wörter nur durch ihre Betonung, beispielsweise \N{\ACC{tu}te} \E{Person} vs. \N{tu\ACC{te}} \E{Frau}.

\subsubsection{} Nur für dieses Wort, \E{Frau}, darf ein Akzent geschrieben werden, um die Betonung anzuzeigen, \N{tuté}.
...
\subsubsection{} Obwohl zusammengesetzte Substantive als ein einziges Wort geschrieben werden, können die einzelnen Bestandteile ihre ursprüngliche Betonung beibehalten, wie in \N{ti\ACC{re}a\ACC{fya}'o} \E{Geisterpfad}.\index{zusammengesetzte Substantive!Betonung}
...
\subsubsection{} Ein Ejektiv am Anfang einer Silbe -- und damit ggf. auch am Anfang eines Wortes -- wird im Riff-Na'vi stattdessen als stimmhafter Plosiv ausgesprochen; d. h. [p' t' k'] → [b d g].
...
\begin{center}
	\begin{tabular}{lll}
		...
		\N{kxitx}   & [git'] & \E{Tod} \\
		\N{skxawng} & [sk'awŋ] (wie Wald-Na'vi) & \E{Idiot, Trottel, Dummkopf}
	\end{tabular}
\end{center}
...
\paragraph{} Wenn ein Wort, das mit einem Ejektiv endet, ein Suffix erhält, das mit einem Vokal beginnt, wird die Aussprache des Ejektivs an die neue Silbenstruktur angepasst. Z. B. ist \N{'awkx} mit dem Ejektiv [ʔawk'], aber \N{'awkxit} ist [ˈʔaw.git], mit der stimmhaften Aussprache.
...
\subsection{Glottaler Plosiv} Der glottale Plosiv wird nicht lenisiert, wenn er einem Pseudovokal vorangestellt ist (\N{mì 'Rrta}, nicht *\N{mì Rrta}).
...
\subsection{Adpositionen} Einige wenige Adpositionen verursachen Lenisierung, wenn sie einem Wort vorausgehen: \N{fpi}, \N{ìlä}, \N{mì}, \N{nuä}, \N{ro}, \N{sko}, \N{sre} (und abgeleitet \N{lisre} und \N{pxisre}), \N{wä} und \N{sru}. Als Suffixe bewirken sie weder in dem Wort, an das sie angehängt sind, noch in dem folgenden Wort eine Lenisierung.
...
\index{Lenisierung!Adpositionen}\index{Adpositionen!Lenisierung}\label{adp:len}
...
\begin{interlin} \label{lenition:ex01}
	...
	\I{1sg} Heim \I{aux} in Heimatbaum \\
...
\index{Trial!Kontraktion}\LNWiki{20/1/2010}{https://wiki.learnnavi.org/index.php/Canon\%23Extracts\_from\_various\_emails}
...
\label{lands:phonotactics:precontract}\index{Nominalpräfix!Kontraktion}\LNWiki{18/5/2011}{https://wiki.learnnavi.org/index.php/Canon/2011/April-December\%23Kawtseng.2C\_tsapo\_and\_prefixes}
...
\subsection{Pseudovokalkontraktion} Aufgrund der Form der Aspektinfixe \N{\INF{er}} und \N{\INF{ol}} ist es möglich, dass die Pseudovokale unmittelbar nach ihrem konsonantischen Gegenstück auftreten, wie in \N{*p\INF{ol}ll\ACC{txe}}. Wenn dies in einer unbetonten Silbe geschieht, verschwindet der Pseudovokal, \N{pol\ACC{txe}}. In einer betonten Silbe verschwindet das Infix, \N{*\ACC{f}\INF{er}\ACC{rr}fen} $>$ \N{\ACC{frr}fen}. Pseudovokale in einsilbigen Wörtern verhalten sich wie unbetont, \N{*v\INF{ol}ll} $>$ \N{vol}. \index{Pseudovokal!Kontraktion}
\LNWiki{23/3/2010}{https://wiki.learnnavi.org/index.php/Canon/2010/March-June\%23Misc\_Answers}
\NTeri{19/6/2012}{https://naviteri.org/2012/06/spring-vocabulary-part-3/}

\subsection{Affektinfix-Epenthese} Wenn auf das positive Affektinfix \N{\INF{ei}} der Vokal \N{i}, \N{ì} oder ein Pseudovokal folgt, wird ein \N{y} eingefügt (Epenthese\footnote{\href{https://de.wikipedia.org/wiki/Epenthese}{Vgl. Wikipedia: Epenthese}}): \N{*s\INF{ei}i} $>$ \N{seiyi}; \N{*veykrr\INF{ei}ìn} $>$ \N{veykrreiyìn}; \N{*veill} $>$ \N{v\INF{ei}yll}.
...
\begin{interlin}
	...
	Entschuldigung \I{aux}\INF{\I{neg.aff}} \I{1sg} \\
...
\subsubsection{} Der Vokal in \N{nì-} entfällt normalerweise vor einem betonten \N{e}, wie in \N{nì-} $+$ \N{etrìp} $>$ \N{netrìp}. Wenn das \N{e} unbetont ist, entfällt es normalerweise (wenn auch nicht immer), \N{nì-} $+$ \N{eyawr} $>$ \N{nìyawr}. Eine Ausnahme: \N{nìean} statt des erwarteten \N{*nìan}.
...
\subsection{Zitation} Direkte Rede wird im Na'vi nicht mit Anführungszeichen gekennzeichnet. Stattdessen wird auf die Zitationspartikel \N{san ... sìk} (\horenref{syn:direct-quote}) zurückgegriffen.
