...
\subsection{-Pe+} \label{morph:pre:pe} Dieses Interrogativaffix bedeutet \E{was, welche/r/s} wie in \N{pelì'u} \E{welches Wort?}. Es ist insofern ungewöhnlich, als es entweder ein Präfix (\N{pelì'u}) oder ein Suffix (\N{lì'upe}) sein kann. Wenn es als Präfix vorangestellt ist, wird das folgende Wort leniert. Wenn auf das Präfix die Pluralsilbe \N{ay+} folgt, kontrahieren die beiden Morpheme zu \N{pay+}. Wird \N{-pe+} als Suffix verwendet, so können daran Fallendungen gehängt werden (z.B. \N{tupel}, \N{tsengpeti}).
