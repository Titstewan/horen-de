...
% For gen. with i: https://forum.learnnavi.org/index.php?msg=150927
...
\index{-o@\textbf{-o}}\index{indefinites Substantiv}\LNWiki{14/3/2010}{https://wiki.learnnavi.org/index.php/Canon/2010/March-June\#A_Collection}
...
\subsubsection{} Wenn die Grundform der ersten Person \N{oe} nicht am Wortende vorkommt, ändert sich ihre Aussprache zu \N{we}, wie in \N{oel} ausgesprochen als \N{wel} [wɛl], \N{oeru} als \N{weru} [ˈwɛ.ɾu]. Diese Aussprache gilt jedoch nicht für die Dual- und Trialformen, \N{moe} und \N{pxoe}, die zu unzulässigen Konsonantenclustern am Wortanfang führen würden, wie etwa *\N{mwel}. Diese Aussprache wird in der Tabelle oben durch die Akzentmarkierung unter dem \N{e} angezeigt. \label{morph:pron:oe-we}
...
\noindent Diese Vokaländerung kommt in allen Numeri vor, \N{fo} $>$ \N{feyä}; \N{awnga} $>$ \N{awngeyä}.
...
\indent Dies gilt auch für die zeremonielle Version \N{poho}, deren Genitiv \N{\ACC{po}heyä} ist (diese Form unterscheidet sich von \N{po\ACC{he}yä}, dem Genitiv des spezifisch femininen zeremoniellen Pronomens, nur durch ihre Betonung). \LNForum{25/10/2022}{https://forum.learnnavi.org/index.php?msg=679687}
...
\begin{center}
	...
	\end{tabular}
\end{center}\index{Pronomen!zeremoniell}\label{morph:hon-pron}\NTeri{28/2/2022}{http://naviteri.org/2022/02/lifyengteri-concerning-honorific-language/}
...
In präziser oder formeller Sprache kann jedoch \N{fìay+} verwendet werden: \label{morph:prenoun:fi}

\begin{interlin}
	\glll \Npawl{}Oel foru fìaylì'ut tolìng a krr, kxawm oe harmahängaw. \\
	...
	\trans{Als ich ihnen diese Worte sagte, schlief ich wohl gerade.}
\end{interlin}
...
{\small\begin{center}
	\begin{tabular}{r|llllll}% name person thing time place manner
		...
		\multirow{2}{*}{alle} & \N{\ACC{fra}po} & \N{\ACC{fra}'u} & \N{frakem}\footnotemark & \N{\ACC{fra}krr} & \N{\ACC{fra}tseng} & \N{\ACC{fra}fya} \\
		& \E{jede/r} & \E{alles (Sache)} & \E{alles (Aktion)} & \E{immer} & \E{überall} & \E{in jeder Weise} \\
...
\footnotetext{\QUAESTIO{Die Betonung dieser Form ist unsicher.}}
...
\subsection{Fragen} In Kombination mit bestimmten Substantiven kann das Interrogativaffix \N{-pe+} (als lenisierendes Präfix oder als Suffix) Fragepronomen bilden: \label{morph:questions}
\begin{center}
	\begin{tabular}{rl|rl}
...
\noindent Genitive wie \N{pesuä} sind den Substantiven ähnlich, d. h. es findet keine Form von Vokalveränderung statt, wie sie bei \N{po} vs. \N{peyä} zu beobachten ist.
...
Beim schnellen Sprechen kann \N{futa} als \N{fta} und \N{tsata} als \N{tsta} ausgesprochen werden.\\
\LNForum{25/2/2022}{https://forum.learnnavi.org/index.php?msg=682691}

\subsection{Andere Substantive für die Einleitung von Inhaltssätzen} Während \N{fì'u} und \N{tsa'u} die meisten Inhaltssätze einleiten können, bevorzugen Verben des Hörens, Sprechens und Befehlens jeweils die Substantive \N{fmawn} \E{Nachricht}, \N{faylì'u} \E{diese Worte} und \N{tson} \E{Pflicht, Verpflichtung} -- die damit gebildeten Inhaltssatzeinleiter kontrahieren ebenfalls und existieren nur im Patiens: \N{fmawnta} ($<$ \N{fmawnit a}), \N{fayluta} ($<$ \N{faylì'ut a}) und \N{tsonta} ($<$ \N{tsonit a}). Siehe \horenref{syn:quot:nominalized} für die Syntax. \NTeri{2/10/2014}{http://naviteri.org/2014/10/tson-si-fpomron-obligation-and-mental-health/}\\ 
\indent Verben des Fragens fordern dagegen das Substantiv \N{tì'eyng} \E{Antwort} im Inhaltssatzeinleiter; dieser kontrahiert und existiert in allen Kasus: \label{morph:fmawn} 
...
\index{tsonta@\textbf{tsonta}}\NTeri{28/12/2024}{https://naviteri.org/2024/12/odds-and-ends/}

\section{Das Adjektiv}
\subsection{Attribution} Attributive Adjektive werden mit dem von ihnen beschriebenen Substantiv durch das Affix \N{-a-} verbunden, das an das Adjektiv auf der Seite angehängt wird, die dem Substantiv am nächsten ist, wie in \N{yerik awin} oder \N{wina yerik} \E{ein schneller Hexapede}.\label{morph:adj-attr}
...
\subsection{Vor-Position} Infixe auf dieser Position ändern die Transitivität. Sie werden vor dem Vokal der vorletzten Silbe eines Verbs eingefügt, oder bei einsilbigen Verben vor den Vokal der einzigen Silbe (Position 0).
...
Das kausative Reflexiv, ,sich selbst (zu etwas) veranlassen‘, wird mit \N{\INF{äp}\INF{eyk}} gebildet, z. B. \N{po täpeykerkup} \E{Er veranlasst sich selbst dazu, zu sterben}. Die Transitivität des ursprünglichen Ausdrucks bestimmt die Kasusverwendung mit Verben, die \N{\INF{äp}\INF{eyk}} enthalten (\horenref{reflexive-of-causative}).

\subsection{Erste Position} Infixe auf dieser Position markieren Tempus, Aspekt und Modus und bilden Partizipien. Sie werden vor dem Vokal der vorletzten Silbe eines Verbs eingefügt (Position 1), oder bei einsilbigen Verben vor den Vokal der einzigen Silbe. Sie folgen immer unmittelbar auf Infixe in der Vor-Position. Die folgende Übersicht zeigt die Zeit- und Aspekt-Infixe: \label{morph:verb:first-position}

\begin{center}
	\begin{tabular}{r|ccc}
		& Nur Tempus & Tempus + Perfekt & Tempus + Imperfekt \\
		\hline
...
\subsubsection{} Es gibt zwei, nicht mit anderen Infixen kombinierbare Partizip-Infixe. Sie lassen sich nicht mit Zeit-, Aspekt- oder Modus-Infixen kombinieren. \index{Partizip!Bildung}
...
\indent Da das Partizip Passiv nur sinnvoll mit transitiven Verben gebildet werden kann, steht \INF{awn} niemals in intransitiven Verben (z. B. wäre ein Satz wie \N{*oe kawnä} \E{Ich werde gegangen} unlogisch). \LNForum{14/8/2014}{https://forum.learnnavi.org/index.php?msg=612540}

\subsection{Zweite Position} Infixe auf dieser Position zeigen die Einstellung oder das Urteil des Sprechers an. Sie werden vor dem Vokal der letzten Silbe eines Verbs eingefügt (Position 2), oder bei einsilbigen Verben vor den Vokal der einzigen Silbe, aber unmittelbar nach den Infixen der Vor-Position bzw. der ersten Position.
